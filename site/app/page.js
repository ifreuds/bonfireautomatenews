  ...
  return (
    <main style={{ maxWidth: 640, margin: '0 auto', padding: '0 16px', fontFamily: '-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif' }}>
      <header style={{ background: '#1a1a2e', borderRadius: 10, padding: '28px 28px', marginTop: 24 }}>
        <div style={{ color: '#e8633a', fontSize: 13, fontWeight: 700, letterSpacing: 1 }}>🔥 BONFIRE SEA CLOUD WEEKLY</div>
        <div style={{ color: '#fff', fontSize: 24, fontWeight: 700, marginTop: 6 }}>The archive</div>
        <div style={{ color: '#aaa', fontSize: 14, marginTop: 6 }}>A weekly read on cloud across Southeast Asia.</div>
        <div style={{ marginTop: 14 }}>
          <Link href="/games" style={{ color: '#e8633a', fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>🎮 Bonfire SEA Games Daily →</Link>
        </div>
      </header>
      ...
      <section style={{ marginTop: 20, paddingBottom: 40 }}>
        ...
        {issues && issues.map((it) => (
          <Link key={it.issue_date} href={`/issue/${it.issue_date}`} style={{ textDecoration: 'none' }}>
